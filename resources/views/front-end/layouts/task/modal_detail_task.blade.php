@foreach($tasks as $key => $task)
    @if(in_array($key, ['tasks_to_do' , 'tasks_in_process', 'task_done', 'task_review']))
        @foreach($task as $index => $dataTask)
            <div class="modal fade modal-dialog-centered" id="{{$key}}_{{$index}}" tabindex="-1"
                    ...
                    <div class="modal-content pt-3 ps-3 pe-3">
                        <div class="rounded-0 text-left">
                            ...
                            <div class="modal-footer" style="border:none">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __("texts.texts.close." . auth()->user()->lang) }}</button>
                                <button type="button" class="btn btn-success">{{ __("texts.texts.update." . auth()->user()->lang) }}</button>
                            </div>
                        </div>
                    </div>
                </div>

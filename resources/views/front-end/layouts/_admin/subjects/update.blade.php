...
@push('js_page')
    <script>
        ...
        saveFormUpdateButton.addEventListener("click", e => {
            ...
            $.ajax({
                url: "/admin/subjects/{{ $subject->uuid }}/update",
                type: "POST",
                data: formData,
                ...
                <div class="row">
                    <div class="col-12 p-5 bg-white rounded-xxl">
                        <div class="plan_header">
                            ...
                            <div class="mt-3">
                                @if ($subject->updatedByUser != null)
                                    <small>
                                        {{ __('texts.texts.last_updated_by.' . auth()->user()->lang) }}
                                        <a href="/users/{{ $subject->updated_by }}">
                                            {{ $subject->updatedByUser->first_name }}
                                            {{ $subject->updatedByUser->last_name }}
                                        </a>
                                ...
                                <form class="row" id="updateSubjectForm" autocomplete="off">
                                    <div class="col-md-6 mt-2">
                                        ...
                                        <input type="text" id="vi" name="vi" class="form-control"
                                            value="{{ json_decode($subject->name, true)['vi'] }}">
                                    </div>
                                    <div class="col-md-6 mt-2">
                                        <label for="en">
                                        ...
                                        <input type="text" id="en" name="en" class="form-control"
                                            value="{{ json_decode($subject->name, true)['en'] }}">
                                    </div>
                                    <div class="col-md-6 mt-2">
                                        <label for="code">
                                        ...
                                        <select name="department" id="department" class="form-control">
                                            ...
                                            @foreach ($departments as $department)
                                                <option value="{{ $department->uuid }}"
                                                    <?php if($subject->department_id == $department->uuid) { ?>selected<?php } ?>>

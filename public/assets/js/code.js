 ...
 *-------------------------------------------------------------
 */
Pusher.logToConsole = messageChat.pusher.debug;
const pusher = new Pusher(messageChat.pusher.key, {
    encrypted: messageChat.pusher.options.encrypted,
    cluster: messageChat.pusher.options.cluster,
    wsHost: messageChat.pusher.options.host,
    wsPort: messageChat.pusher.options.port,
    wssPort: messageChat.pusher.options.port,
    forceTLS: messageChat.pusher.options.useTLS,
    authEndpoint: messageChat.pusherAuthEndpoint,
  auth: {
    headers: {
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").text(data.fetch.last_name + data.fetch.first_name);
        $(".m-header-messaging .user-name").text(data.fetch.last_name + data.fetch.first_name);
        // Star status
        data.favorite > 0
...
$(document).ready(function () {
  ...
  function attachmentValidate(file) {
    ...
    if (
      !messageChat.allAllowedExtensions.includes(
    ...
    // Validate file size.
    if (fileSize > messageChat.maxUploadSize) {
